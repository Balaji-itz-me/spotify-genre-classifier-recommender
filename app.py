...
def get_model_features(scaler_model):
    ...
    try:
        if hasattr(scaler_model, 'feature_names_in_'):
            ...
            # Fallback to common features if not available
            return ['danceability', 'energy', 'loudness', 'speechiness', 
...
def create_complete_feature_vector(user_inputs, required_features, df_sample=None):
    ...
    defaults = {
        ...
        'time_signature': 4,
        'cluster': 0,
        'popularity': 50,  # In case it's needed
    }
    ...
    if df_sample is not None:
        for feature in required_features:
            if feature in df_sample.columns and feature not in user_inputs:
                try:
                    defaults[feature] = df_sample[feature].median()
                except:
                    pass
    ...
    for feature in required_features:
        if feature in user_inputs:
            ...
            complete_features[feature] = [0]  # Last resort default
...
def popularity_module():
    ...
    if st.button("Predict Popularity", type="primary"):
        try:
            ...
            required_features = get_model_features(models['scaler'])
            ...
            with st.expander("🔍 Features Used"):
                st.write(f"Model trained on {len(required_features)} features:")
                st.write(", ".join(required_features[:10]) + ("..." if len(required_features) > 10 else ""))
