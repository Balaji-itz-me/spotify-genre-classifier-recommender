...
def download_and_extract_files():
    ...
    try:
        ...
        if extracted_folders:
            ...
            st.info(f"🔧 Debug: Using subfolder as base: {base_dir}")
        else:
        ...
        st.info(f"🔧 Debug: Files in base directory: {base_files[:10]}")  # Show first 10 files
...
def load_data():
    ...
    try:
        ...
        tsne_data = pd.read_pickle(os.path.join(base_dir, "tsne_3d_cluster.pkl"))
        pca_data = pd.read_pickle(os.path.join(base_dir, "pca_3d_cluster.pkl"))
